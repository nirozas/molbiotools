...
export default function AnalysisForm({ onRun, loading, onClassChange }: AnalysisFormProps) {
    ...
    const fetchAlleles = async () => {
        try { const res = await axios.get(`${API_BASE}/alleles`, { params: { mhcClass } }); setAllAlleles(res.data); }
        catch (err) { console.error("Failed to fetch alleles"); }
    };

    const filteredAlleles = useMemo(() =>
        allAlleles.filter(a => organism.prefixes.some(p => a.startsWith(p)) && a.toLowerCase().includes(alleleQuery.toLowerCase())).slice(0, 80),
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
            ...
            <SectionCard accent={accent}>
                ...
                <div ref={alleleRef} style={{ position:'relative', marginBottom:'0.75rem' }}>
                    ...
                    <AnimatePresence>
                        {showAlleleList && filteredAlleles.length > 0 && (
                            <motion.div initial={{ opacity:0, y:-5 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-5 }}
                                style={{ position:'absolute', top:'calc(100% + 5px)', left:0, right:0, background:'rgba(6,12,28,0.98)', border:`1px solid ${accent}30`, borderRadius:'10px', zIndex:100, maxHeight:'220px', overflowY:'auto', backdropFilter:'blur(20px)', boxShadow:`0 20px 50px rgba(0,0,0,0.6)` }}
                                    ...
                                    {filteredAlleles.map(a => {
                                        ...
                                        return (
                                            <button key={a} type="button"
                                                ...
                                                style={{ width:'100%', padding:'0.5rem 0.8rem', borderRadius:'7px', textAlign:'left', background: sel ? `${accent}18` : 'transparent', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'space-between' }}
                                                onMouseEnter={e => { if(!sel) (e.currentTarget as HTMLElement).style.background=`${accent}0e`; }}
                                                onMouseLeave={e => { if(!sel) (e.currentTarget as HTMLElement).style.background='transparent'; }}>
                                                <span style={{ fontSize:'0.8rem', fontWeight: sel?700:500, color: sel?accent:'#94a3b8' }}>{a}</span>
                                                {sel && <Check size={12} style={{ color:accent }} />}
                                            </button>
                                        );
                                    })}
                                </div>
                                <div style={{ padding:'0.35rem 0.8rem 0.5rem', borderTop:`1px solid rgba(148,163,184,0.06)`, fontSize:'0.65rem', color:'#334155', fontWeight:600 }}>
                                    {filteredAlleles.length} alleles shown · click to select
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
